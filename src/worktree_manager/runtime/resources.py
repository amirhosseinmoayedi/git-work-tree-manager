...
def allocate(cfg: dict[str, Any], registry, values: dict[str, Any]) -> dict[str, Any]:
    ...
    for name, spec in cfg.get("resources", {}).get("ports", {}).items():
        if spec.get("strategy", "first_free") != "first_free":
            raise ValueError(f"unsupported port strategy for {name}")
        for p in range(int(spec.get("from", 8000)), int(spec.get("to", 8999)) + 1):
            if p not in used and port_free(p):
                resources["ports"][name] = p
                used.add(p)
                break
        else:
            raise RuntimeError(f"no free port for {name}")
